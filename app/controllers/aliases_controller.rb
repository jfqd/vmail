class AliasesController < ApplicationController
  ...
  def index
    if params[:domain]
      @aliases = Alias.paginate_by_domain_id params[:domain], :page => params[:page], :order => 'domain_id asc'
    else
      @aliases = Alias.paginate :page => params[:page], :order => 'domain_id asc'
    end
  ...
  def edit
    ...
    @alias.name = @alias.address.split("@")[0]
  end
